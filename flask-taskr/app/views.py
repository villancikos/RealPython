...
def register():
	...
	if form.validate_on_submit():
		...
		try:
			...
			error = "Oh nopes! That username and/or email already exist. Please Try again."
...
def display_task(task_id):
	...
	# g.db = connect_db()
	# cur = g.db.execute('select name,due_date,priority,task_id from ftasks where task_id='+str(task_id))
	# task = [dict(name=row[0], due_date=row[1], priority=row[2],task_id=row[3]) for row in cur.fetchall()]
	# g.db.close()
	# return render_template('task.html',task = task)
...
def internal_error(error):
	db.session.rollback()
	return render_template('500.html'),410
